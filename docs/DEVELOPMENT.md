...
| `pnpm test` | Run `@parawrite/core` and `@parawrite/server` tests |
| `pnpm clean` | Remove build outputs, beta artifacts, and tsbuildinfo files |
...
`pnpm clean` removes all `dist/` outputs, `artifacts/parawrite-beta/`, and TypeScript build info files. Run `pnpm build` (or `pnpm dev`) before `pnpm lint` if you cleaned a fresh checkout.
...
pnpm test        # Core + server tests (tsx, from source)
