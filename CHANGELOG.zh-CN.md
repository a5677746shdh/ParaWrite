...
### 新增
- 源/译文栏术语表标注（`point_out_glossary`：`off` | `first` | `full`）
- `dictionary.llm_show_examples`（默认 `false`）— 关闭时 LLM 词典不请求/不显示例句
- 按选中 token 位置解析备选表达从句（支持译文中重复词）
- `alternatives_separator` 同时控制备选表达选择范围；`comma` 模式含句末标点

### 变更
- 移除查词栏客户端缓存，避免同义词/备选表达结果串用
- 扩展从句/句末标点集合（全角逗号、冒号、省略号、`?`/`!` 等）
- 切换界面语言时刷新当前选词的词典

### 修复
- 备选表达替换整段目标从句，而非仅替换点击的词
- 备选表达带句末标点时避免与后文标点重复
- 重复词（如两个 `are`）不再共用第一次查词时的备选表达
- 用户取消选词/换词时的请求中断不再误报模型 API 错误日志

## [0.8.1] — 2026-06-27

### 新增
- 回译预加载三档：`rephrase_back_translation_preload`（`off` | `partial` | `all`）
- 译文栏底部布局：查词按钮居左、字数居中；窄/宽栏自适应
- 查词栏客户端缓存，重复查同一选区免网络请求
- `/api/meta` 返回用户 `locale`，登录启动时减少一次 `/api/user/me`

### 变更
- 「已添加到收藏」提示居右并保持至原文/译文变化；期间收藏按钮禁用
- 回译预加载最多 3 路并发
- 语言自动检测与自动交换合并为一次防抖检测
- 后端用户配置/术语表缓存（mtime TTL）；历史查询使用预编译语句
- 词典双语 LLM 结果缓存并支持请求 abort

### 修复
- 自动语言交换后不再重复触发翻译
- 历史去重索引补全 `target_text`
...
### 新增
- 备选表达悬停回译预览（可配置开关与延迟），差异词高亮
- 备选表达批量回译按钮（展开/收起全部回译）

### 变更
- 批量回译按钮在原文语言确定后即显示，与悬停回译开关无关

### 修复
- 三栏切两栏且已选词时，查词栏改为弹窗显示而非隐藏
- 布局切换后原文输入框高度重新计算，避免内容被截断

## [0.7.1] — 2026-06-25

### 新增
- 按语言对配置原文/译文栏宽度比例（`app.layout.pane_width_ratios`）
- 译文栏已有内容时，切换目标语言自动重新翻译
- 根目录 `pnpm test`、`pnpm clean` 脚本

### 变更
- 历史面板不再随翻译流式输出逐块重渲染（收藏按钮独立订阅）
- 登录失败计数 24 小时后自动清理
- 注册用户名提示仅在输入无效后出现，关闭弹窗前保持显示

### 修复
- 改写应用失败时不再误替换整段译文
- 同义词替换后取消选词高亮（与备选表达一致）
- 关闭 `model_api_errors` 时不再误屏蔽通用 API 错误日志
- 目标语言切换效果不再在流式输出每个分块时重复触发

### 移除
- 未使用的剪贴板导出；粘贴请用 `pasteFromClipboard`
- 空遗留目录（`users/`、`db/`、`routes/`）
